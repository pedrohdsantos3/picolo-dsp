...
- `controller/FxParameterController` e `controller/CabinetParameterController`
  ...
  injetadas. O controller interno ainda mantém operações de ciclo de vida do
  áudio, importação, remoção e reordenação das cadeias.
...
2. **Controlador da aplicação:** casos de uso coordenam presets, roteamento,
   ...
   extraídos para controllers testáveis. Falta mover o restante de
